...
def obter_ratings_omdb(imdb_id):
    ...
    try:
        ...
        if response.status_code != 200:
            logger.error(f"Erro OMDB API: status {response.status_code}")
            return None

        data = response.json()

        if data.get('Response') == 'False':
            logger.warning(f"OMDB não encontrou filme: {data.get('Error')}")
            return None
        ...
        logger.error(f"Erro ao buscar ratings OMDB para {imdb_id}: {str(e)}")
        return None
